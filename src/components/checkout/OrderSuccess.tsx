  ...
  if (!order) {
    return (
      <div className="mx-auto flex max-w-xl flex-col items-center gap-4 px-4 py-24 text-center sm:px-6">
        <p className="font-display text-2xl text-cream">Nessun ordine recente</p>
        <p className="text-sm text-muted-foreground">
          Non troviamo un ordine da mostrare. Esplora la collezione per iniziare un nuovo acquisto.
        </p>
        <Link
          href="/catalog"
    ...
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="mt-10 rounded-md border border-border bg-card p-6 sm:p-8">
        <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
          <div>
            <p className="text-xs uppercase tracking-wide text-muted-foreground">Numero Ordine</p>
        ...
        <div className="space-y-2.5 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-cream">
              {order.shipping === 0 ? "Gratuita" : money(order.shipping)}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <span className="font-display text-lg text-cream">Totale Pagato</span>
          <span className="font-display text-2xl text-gold">{money(order.total)}</span>
        </div>

        <div className="mt-8 grid gap-6 sm:grid-cols-2">
          <div>
            <p className="text-xs uppercase tracking-luxe text-gold">Spedizione</p>
          ...
          <div>
            <p className="text-xs uppercase tracking-luxe text-gold">Pagamento</p>
            <p className="mt-2 text-sm text-muted-foreground">
      ...
      <div className="mt-10 flex flex-col items-center gap-4 text-center">
        <p className="text-xs text-muted-foreground">
          Progetto demo di portfolio — questo ordine è simulato e nessun pagamento reale è stato
          elaborato.
        </p>
