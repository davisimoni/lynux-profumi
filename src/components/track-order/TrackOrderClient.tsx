...
export function TrackOrderClient() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (normalized.length === 0) {
      setError(t.trackOrder.codeRequired);
      return;
    }
    setError(null);
    setSubmittedCode(normalized);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-14 sm:px-6 lg:px-8">
      <div className="mb-10 flex flex-col items-center gap-3 text-center">
        ...
        <p className="max-w-md text-sm text-muted-foreground">{t.trackOrder.subtitle}</p>
      </div>
      ...
      {!submittedCode && (
        ...
        >
          <label htmlFor="order-code" className="text-xs uppercase tracking-wide text-muted-foreground">
            {t.trackOrder.codeLabel}
          </label>
          <input
            id="order-code"
            value={inputCode}
            onChange={(e) => setInputCode(e.target.value)}
            placeholder={t.trackOrder.codePlaceholder}
          ...
          {mounted && lastOrder && (
            ...
            >
              {t.trackOrder.useLastOrder(lastOrder.orderNumber)}
            </button>
          )}
        </form>
      )}
      ...
      {submittedCode && !tracking && (
        <div className="flex flex-col items-center gap-3 rounded-md border border-border bg-card py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">{t.trackOrder.verifying}</p>
        </div>
      )}
      ...
      {tracking && (
        <div className="space-y-8">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-border bg-card px-5 py-4">
            <div>
              <p className="text-xs uppercase tracking-wide text-muted-foreground">{t.trackOrder.order}</p>
          ...
          {!serverOrder && !localOrder && (
            <p className="rounded-sm border border-dashed border-border px-4 py-3 text-center text-xs text-muted-foreground">
              {t.trackOrder.notFoundOnBackend}
            </p>
          )}
          {!serverOrder && localOrder && (
            <p className="rounded-sm border border-dashed border-border px-4 py-3 text-center text-xs text-muted-foreground">
              {t.trackOrder.foundLocalOnly}
            </p>
          )}
          ...
          <div className="rounded-md border border-border bg-card p-6 sm:p-8">
            <ol className="space-y-0">
              {TRACKING_STAGES.map((stage, index) => {
                ...
                const stageText = t.trackOrder.stages[stage.key as keyof typeof t.trackOrder.stages];
                      ...
                      >
                        ...
                        {isCurrent && (
                          <span className="ml-2 text-xs uppercase tracking-wide text-gold">
                            {t.trackOrder.inProgress}
                          </span>
                        )}
                      </p>
                      <p className="mt-1 text-sm text-muted-foreground">{stageText.description}</p>
                      <p className="mt-1 text-xs text-muted-foreground/70">
                        {isFuture ? t.trackOrder.scheduled : formatStageDate(stageDate)}
                      </p>
                    </div>
                  </li>
                );
          ...
          {summaryItems && summaryTotal !== null && (
            <div className="rounded-md border border-border bg-card p-6 sm:p-8">
              <p className="mb-4 text-xs uppercase tracking-luxe text-gold">{t.trackOrder.orderSummary}</p>
              ...
              <div className="mt-4 flex items-center justify-between border-t border-border pt-4">
                <span className="font-display text-cream">{t.trackOrder.totalPaid}</span>
